...
bool Ack_receiver::add_slave(THD *thd)
{
  ...
  {
    mysql_mutex_unlock(&m_mutex); //no cover line
    return function_exit(kWho, true); //no cover line
  }
  return function_exit(kWho, false);
}
...
{
  // TODO: define the stage and call: set_stage_info(stage_waiting_for_semi_sync_slave);
  mysql_cond_wait(&m_cond, &m_mutex);
}
...
void Ack_receiver::run()
{
  ...
  while (1)
  {
    ...
    //TODO: set_stage_info(stage_waiting_for_semi_sync_ack_from_slave);
    ...
    // TODO: set_stage_info(stage_reading_semi_sync_ack);
    i= 0;
